...
| `BREVO_API_KEY` | En producción | API key de [brevo.com](https://brevo.com) (300 emails/día gratis). Si está definida, **todos los emails salen por su API HTTP** y se ignora la config SMTP. **Hace falta desplegado**: Render bloquea el tráfico saliente a los puertos SMTP 25/465/587 en los web services del plan free (desde el 2025-09-16), así que ahí nodemailer muere con `ENETUNREACH` aunque las credenciales estén bien. `MAIL_FROM` tiene que ser una dirección **verificada** en Brevo (se carga en el panel y se responde un código); un Gmail sirve, no hace falta dominio propio. Se eligió Brevo sobre Resend justamente por eso: Resend exige dominio verificado para escribirle a alguien que no sea el dueño de la cuenta. |
