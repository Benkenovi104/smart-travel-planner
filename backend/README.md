...
| `RESEND_API_KEY` | En producción | API key de [resend.com](https://resend.com) (3.000 emails/mes gratis). Si está definida, **todos los emails salen por su API HTTP** y se ignora la config SMTP. **Hace falta desplegado**: Render bloquea el tráfico saliente a los puertos SMTP 25/465/587 en los web services del plan free (desde el 2025-09-16), así que ahí nodemailer muere con `ENETUNREACH` aunque las credenciales estén bien. `MAIL_FROM` tiene que ser de un dominio verificado en Resend; una dirección de Gmail la rechaza con 403. |
