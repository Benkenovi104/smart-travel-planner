...
|---|---|---|
| `auth` | `POST /api/auth/register`, `POST /api/auth/login`, `POST /api/auth/change-password`, `POST /api/auth/forgot-password`, `POST /api/auth/reset-password` | Registro/login con JWT (bcrypt, 7 días). Cambio de contraseña (autenticado) y recuperación por email (token SHA-256 con vencimiento de 1h). `forgot-password` responde **siempre** el mismo mensaje genérico —exista o no el email, y aunque falle el envío del mail—: si el fallo de SMTP devolviera 500, sólo lo haría para emails registrados y eso permitiría enumerar cuentas. El error queda en el log del servidor. Rate limit de 5 intentos/minuto. |
